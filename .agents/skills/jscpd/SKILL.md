...
**These passes are noisy by design.** An exact clone is almost always a real copy. A renamed or similar clone is a _candidate_: the flags deliberately ignore the very things (names, values, a statement or two) that often make two blocks different in meaning. Expect false positives from:
